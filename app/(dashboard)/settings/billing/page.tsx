...
export default function BillingPage() {
  ...
  return (
    <div className="space-y-5 max-w-xl">
      ...
      {/* ── Invoices (own section) ───────────────────────────────────────── */}
      <section className="pt-3">
        <div className="flex items-center gap-2 mb-2.5">
          <div className="w-7 h-7 rounded-lg bg-slate-100 flex items-center justify-center">
            <Receipt className="w-4 h-4 text-slate-500" strokeWidth={2.2} />
          </div>
          <div>
            <h2 className="text-[15px] font-bold text-ink leading-tight">Invoices</h2>
            <p className="text-[11.5px] text-slate-500">Download your Leadkaun invoices and receipts.</p>
          </div>
        </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wide text-slate-400 border-b border-slate-100">
                    ...
                    <th className="px-4 py-2.5" aria-label="Download" />
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {history.map((h) => (
                    <tr key={h.id} className="text-[12.5px]">
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                        {new Date(h.at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                      </td>
                      <td className="px-4 py-3 text-slate-500 tabular-nums">{h.number ?? "—"}</td>
                      <td className="px-4 py-3 text-right font-medium text-ink whitespace-nowrap">
                        {rupees(h.amountInr)}
                      </td>
                      <td className="px-4 py-3 text-right whitespace-nowrap">
                        <span className={`capitalize ${HISTORY_STATUS[h.status] ?? "text-slate-500"}`}>
                          {h.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        {h.kind === "invoice" && h.downloadable && (
                          ...
                          >
                            <Download className="w-3.5 h-3.5" strokeWidth={2.2} /> PDF
                          </a>
                        )}
                      </td>
                    </tr>
