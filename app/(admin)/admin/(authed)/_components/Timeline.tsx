      ...
      {events.map((e) => {
        ...
        return (
          <li key={e.id} className="ml-4">
            <span className={`absolute -left-[5px] mt-1.5 w-2.5 h-2.5 rounded-full ${s.dot}`} aria-hidden="true" />
            {/* The dot is the only indicator of event type for a sighted reader. */}
            <span className="sr-only">{s.label}: </span>
            <div className="flex items-baseline justify-between gap-3">
              <p className="text-[12.5px] text-ink-soft min-w-0">
                <span className="text-[9.5px] font-semibold uppercase tracking-wider text-ink-muted mr-2">{s.label}</span>
                {e.summary}
                {showAccount && e.accountName && (
                  <Link href={`/accounts/${e.accountId}`} className="text-sky-600 hover:text-sky-700 font-semibold ml-1.5">
                    · {e.accountName}
                  </Link>
                )}
              </p>
